class TabLink {
  constructor(tabElement) {
    ...
    /*
     ...
     // statement above. Convert each this.cards element into a new instance of
     // the TabCard class. Pass in a card object to the TabCard class.*/
    this.cards = Array.from(this.cards).map(card => new TabCard(card));

    // Add a click event that invokes this.selectTab

    this.tabElement.addEventListener("click", this.selectTab.bind(this)); // Could also: () => this.selectTab()
  }
  ...
  selectTab() {
    ...
    // this.cards was returned the TabCard class object (well, for all the ones we iterated, anyway)
    // so each object has access to the TabCard methods
  }
}
